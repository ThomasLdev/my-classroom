...
export default class extends Controller {
    ...
    connect() {
        ...
        this.selectedIndex = this.cells.findIndex((c) =>
            c.classList.contains("weekstrip__cell--selected"),
        );
    ...
    onTouchStart(event) {
        if (event.target.closest(".weekstrip__cell--selected"))
            event.stopPropagation();
    }
    ...
    onPointerMove(event) {
        ...
        const translate = Math.max(
            0,
            Math.min(this.maxIndex * this.step, this.baseTranslate + dx),
        );
        ...
        if (nearest !== this.targetIndex) {
            ...
            this.cells.forEach((cell, i) => {
                cell.classList.toggle("weekstrip__cell--target", i === nearest);
            });
        }
    }
    ...
    onPointerUp() {
        ...
        this.cells.forEach((cell) => {
            cell.classList.remove("weekstrip__cell--target");
        });
